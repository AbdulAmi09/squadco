...
import { hashSensitiveValue } from "./hash.service.js";
import { captureException, captureMessage } from "./monitoring.service.js";
import { applyTrustScoreChange } from "./trust-score.service.js";
import { delKey, getJson, setJson } from "./upstash.service.js";
import { fireWebhookEvent } from "./webhook.service.js";
...
async function getCustomerBaseline(customerId: string) {
  ...
  const avgAmount = history.length
    ? history.reduce((sum, item) => sum + Number(item.amount), 0) / history.length
    : 1;
...
export async function registerCustomer(orgId: string, payload: {
  external_id: string;
  bvn_hash?: string;
  phone_hash?: string;
}) {
...
export async function analyzeTransaction(orgId: string, payload: {
  ...
  if (analysis.decision === "allow") {
    ...
    await applyTrustScoreChange({
      ...
      orgId,
      changeAmount: 5,
      reason: "Successful transaction with no flags",
  ...
  if (analysis.decision === "block") {
    await applyTrustScoreChange({
      ...
      orgId,
      changeAmount: -30,
      reason: "Transaction blocked by security engine",
      metadata: { transaction_id: transaction.id }
    });
  }
  ...
  if (analysis.risk_factors.some((factor) => factor.type === "high_velocity")) {
    await applyTrustScoreChange({
      ...
      orgId,
      changeAmount: -15,
      reason: "High velocity pattern detected",
      metadata: { transaction_id: transaction.id }
    });
  }
  ...
  void aiEngineService.explain<{ explanation: string; model_used?: string; fallback_level?: number }>({
    ...
    .catch(async () => {
      const fallbackExplanation = "This transaction was flagged by our security system. Please contact your bank if you believe this is an error.";
      await prisma.transaction.update({
        where: { id: transaction.id },
        data: {
      ...
      await captureMessage("Risk explanation fallback applied", "warning", {
        orgId,
...
export async function analyzeCredit(orgId: string, payload: {
  customer_id: string;
  data_type: string;
  ...
  const inputs = await prisma.creditInput.findMany({
    where: {
      orgId,
      customerId: payload.customer_id
    }
  ...
  const scored = await aiEngineService.scoreCredit<CreditResult & { source_notes?: Record<string, string | null> }>({ sources });
  ...
  try {
    ...
    improvementTips = tips.explanation
      .split(/\n|;/)
    ...
    await captureMessage("Credit improvement tips fallback applied", "warning", {
      orgId,
      customerId: payload.customer_id
    });
  }
  ...
  await prisma.bankCustomer.update({
    where: { id: payload.customer_id },
    data: { creditScore: scored.credit_score }
  });

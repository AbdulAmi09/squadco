...
async function post<T>(path: string, payload: unknown, options?: { timeoutMs?: number; retryOnce?: boolean; requestId?: string }): Promise<T> {
  ...
  try {
    const response = await fetch(`${env.aiEngineUrl}${path}`, {
      ...
      headers: {
        ...
        "x-internal-secret": env.aiEngineSecret,
